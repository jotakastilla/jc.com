...
const allowedPrices = new Set(Object.values(academyConfig.priceIds));
...
export async function POST(request: Request) {
  ...
  const form = new URLSearchParams({ mode: "payment", "line_items[0][price]": priceId, "line_items[0][quantity]": "1", success_url: `${origin}/pago-confirmado?session_id={CHECKOUT_SESSION_ID}`, cancel_url: `${origin}/premium` });
