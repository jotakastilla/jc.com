...
export default async function ContentPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const guide = <>{lesson.article && <div className={styles.article}>{lesson.article.map((section) => <section key={section.heading}><h2>{section.heading}</h2>{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>)}</div>}{mobileSetup}{episodeMap}<div className="lessonGrid"><section><h2>Tu primer paso a paso</h2><ol>{lesson.steps.map((step) => <li key={step}>{step}</li>)}</ol></section><aside><small>PARA EMPEZAR</small><h3>Kit mínimo</h3><ul>{lesson.starterKit.map((item) => <li key={item}>{item}</li>)}</ul></aside></div><Link className="textLink" href="/areas#podcast">← Ver más contenidos</Link></>;
